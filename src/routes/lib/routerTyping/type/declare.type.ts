...
import type { NormalizePath } from '@/routes/lib/routerTyping/type/utils.type';
...
export type ToRouteUnion<T extends IRoutes> =
  T extends ReadonlyArray<infer I> ? MergeChild<I> : never;
/**
 ...
 * }
 */
export type AsObjectShape<T extends { path: string }> = {
  [K in T['path']]: {
    path: K;
...
export type MergeChild<T> = T extends {
  path: string;
  ...
  ?
      ...
      | MergeChild<
          {
            path: NormalizePath<`${T['path']}/${Children['path']}`>;
 ...
 */
export type ParsePathParams<T extends string> = [T] extends [
  `${string}:${infer I1}`,
]
